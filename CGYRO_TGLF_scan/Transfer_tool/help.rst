...
界面、日志及 OUTPUTS/Particle_processing/main_ions 显示原始编号和平均占比。
...
* 仅保留主离子和等效杂质：固定电子密度和初次准中性校正后的 Zeff，保留全部主离子和一种等效杂质。主离子保持各半径的原密度比例，分别沿用温度和流速。杂质质量、电荷取所选原粒子，温度及流速采用平均 ni/ne 最高的主离子值；原剖面已全部为主离子时不新增杂质。
* 仅保留主离子：保留电子和全部主离子，各半径按原主离子密度比例补齐电荷，分别保留各自温度和流速。

所有方案均保留电子。自动等效杂质取电荷最高的非主离子，也可用下拉框明确指定。
其他来源在生成 input.gacode 后自动识别；可切到该 input.gacode 再指定杂质或热化参考。
...
主离子组按电荷加权的电荷为 ZM，随半径变化：

.. math::

   Z_M = \frac{\sum_{m\in M} Z_m^2 n_m}{\sum_{m\in M} Z_m n_m},
   \qquad n_q = \frac{n_e (Z_{eff} - Z_M)}{Z_q (Z_q - Z_M)}

全部密度须非负；若 Zq=ZM，只有目标 Zeff 也相同时可保持被替换粒子的总电荷密度。
所选杂质电荷不适合时停止，提示重新选择或保留所有粒子。
...
切换粒子方案或主离子识别半径后必须重新生成，旧版单主离子结果也须重新生成后才能传递。
