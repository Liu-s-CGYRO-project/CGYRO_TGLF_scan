...
if root['SETTINGS']['PHYSICS']['start_from'] == 'ODS':
    ...
    if profpowbal is None or not isinstance(profpowbal, ODS):
        OMFITx.CheckBox("scratch['input_ok_as_output']", '载入 ODS 并作为剖面生成结果', default=True)
    ...
    if root['SETTINGS']['PHYSICS']['start_from'] == 'statefile':
        OMFITx.CheckBox("root['SETTINGS']['PHYSICS']['use_trxpl']", '使用 TRXPL', default=False, updateGUI=True)
        ...
        if 'statefile' not in root['INPUTS'] and (profpowbal is None or not isinstance(profpowbal, OMFITnc)):
            OMFITx.FilePicker("scratch['statefile_fn']", '功率平衡文件', updateGUI=True, postcommand=loadstate, default='')
            ok = False
            ...
            OMFITx.Button('更换状态文件', delstate, updateGUI=True)
    ...
    if 'gEQDSK' not in root['INPUTS'] and gEQDSK is None:
        if root['SETTINGS']['PHYSICS']['start_from'] == 'pfile':
            OMFITx.FilePicker("scratch['geqdsk_fn']", '平衡文件', updateGUI=True, postcommand=loadg, default='')
            ok = False
        else:
            OMFITx.FilePicker("scratch['geqdsk_fn']", '平衡文件（可选）', updateGUI=True, postcommand=loadg, default='')
    ...
    if 'CER' not in root['INPUTS'] and CER is None:
        OMFITx.FilePicker("scratch['cer_fn']", 'CER 文件（可选）', updateGUI=True, postcommand=load_cer, default='')
        ...
        OMFITx.Button('更换 CER 文件', del_cer, updateGUI=True)
    ...
    if 'input.gacode' not in root['INPUTS'] and (profpowbal is None or not isinstance(profpowbal, OMFITgacode)):
        OMFITx.FilePicker("scratch['input.gacode_fn']", 'input.gacode 输入文件', updateGUI=True, postcommand=load_input_gacode, default='')
        OMFITx.CheckBox("scratch['input_ok_as_output']", '载入 input.gacode 并作为剖面生成结果', default=True)
        ok = False
        ...
        OMFITx.Button('更换 input.gacode', del_input_gacode, updateGUI=True)

if ok:
    OMFITx.CompoundGUI(root['GUIS']['vgenGUI'], showRunButton=True)

OMFITx.Button('绘制所选输入', "root['PLOTS']['plot_input'].runNoGUI")
