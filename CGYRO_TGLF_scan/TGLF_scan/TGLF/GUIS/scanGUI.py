...
if 'input.tglf' in root['FILES']:
    ...
    for param, desc in param_desc:
        ...
        for ic, c in enumerate(charges, 1):
            ...
            if c == -1:
                spec = 'Electron'
            if c == 1:
                if abs(masses[ic - 1] - 1) < 1e-3:
                    spec = 'Deuterium'
                elif abs(masses[ic - 1] - 0.5) < 1e-3:
                    spec = 'Hydrogen'
                elif abs(masses[ic - 1] - 1.5) < 1e-3:
                    spec = 'Tritium'
            if c == 6:
                spec = 'Carbon'
            if c == 2:
            ...
            if spec != 'Electron':
                ...
                if (param == 'RLTS_' or param == 'TAUS_') and root['SETTINGS']['PHYSICS']['single_Ti'] and nc == 1:
                    if root['FILES']['input.tglf']['%s%d' % (param, ic)] == root['FILES']['input.tglf']['%s2' % (param)]:
                        spec = "Thermal ions'"
                choices['%s %s (%s)' % (spec, desc, param + str(ic))] = param + str(ic)
            elif nc > 2:
    ...
    OMFITx.Entry(
        "root['SETTINGS']['PHYSICS']['scanParameter%sMin']" % paramN,
        'Minimum factor (%g)' % (value * root['SETTINGS']['PHYSICS']['scanParameter%sMin' % paramN]),
        updateGUI=True,
    )
    OMFITx.Entry(
        "root['SETTINGS']['PHYSICS']['scanParameter%sMax']" % paramN,
