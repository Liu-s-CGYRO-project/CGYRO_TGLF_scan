...
class ComparisonWidgets:
    ...
    def _render_index_checkbox_grid(self, path_template, values, cols=4):
        for begin in range(0, len(values), cols):
            with self.ui.same_row():
                for idx in range(begin, min(begin + cols, len(values))):
                    self.ui.CheckBox(path_template.format(idx=idx), str(values[idx]),
                                     default=False, updateGUI=True)

    def _render_key_checkbox_grid(self, path_builder, values, cols=4):
        for begin in range(0, len(values), cols):
            with self.ui.same_row():
                for key in values[begin:begin + cols]:
                    self.ui.CheckBox(path_builder(key), str(key), default=False, updateGUI=True)
    ...
    def _render_parameter_entries(self, state, flag_key, selected_key, parameters,
        ...
        for begin in range(0, len(parameters), 4):
            with self.ui.same_row():
                for idx in range(begin, min(begin + 4, len(parameters))):
                    name = parameters[idx]
        ...
        for idx, name in enumerate(parameters):
            if flags.get(idx):
                ...
                self.ui.Entry(entry_path_builder(name), '{} values'.format(name),
                              default=selected[name], updateGUI=False)
            else:
                selected.pop(name, None)
        if not parameters:
            self.ui.Label('No common scan parameters for the selected cases.', align='left')
        return selected
        ...
        for idx, name in enumerate(para_list):
            ...
            if spectra_mode == '2D':
                ...
                for field, title in [('para1_values', p1_name), ('para2_values', p2_name)]:
                    self.ui.Entry(path_builder('TGLF', key, name, field),
                                  '{}: {} values'.format(name, title), default=cfg.get(field, []))
            else:
                selected.setdefault(name, self._normalize_value_list(k for node in nodes for k in node.keys()))
                self.ui.Entry(path_builder('TGLF', key, name), '{} values'.format(name), default=selected[name])
        if not para_list:
            self.ui.Label('No common TGLF parameters for the selected radii.', align='left')
    ...
    def _render_legend_order_controls(self, path_builder, state, default_legend_order='', editor_title='Legend order'):
        ...
        if state.get('legend_order_mode') == 'Manual order':
            ...
            self.ui.Button('Reorder list...', lambda: self._edit_legend_order(state, default_legend_order, editor_title))
        self.ui.Label('Click a legend entry to hide/show that curve in the figure.', align='left')
