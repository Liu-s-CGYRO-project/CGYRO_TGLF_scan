# OMFIT 兼容审计与内部支持代码 · 1.1.1

## 1.1.1 / 管理器 1.3.1：连接脚本的 Python 环境

桌面通过绝对路径启动 OMFIT 时，子进程继承的 PATH 可能找不到 python3。加载脚本现在仅对子进程将 `sys.executable` 所在目录置于 PATH 首位，不解析虚拟环境符号链接，不写入个人绝对路径，不修改 OMFIT 主进程环境。该修复要求 OMFIT 环境提供 python3；外部终端仍需先激活相应环境。

120 项 Linux 管理器回归通过（10 项代理/脚本测试），其中真实 bash 测试故意清除 PATH 中的 python3，再验证加载器使用当前 Python 环境执行脚本。错误提示区分启动错误码、脚本退出状态与 30 秒超时，凭据不进入错误文字。17 项原生执行检查及全量生产源码审计重新通过。GUI、科学计算代码未改动，相关界面与科学计算回归分别沿用 1.1.0 和 1.0.1。

记录见 `validation/proxy_1_1_1/`。没有服务器脚本和会话，本地复现验证不等同于已连通 tyadmin06 的真实 SSH 中继；未运行完整服务器 OMFIT 或求解器。
