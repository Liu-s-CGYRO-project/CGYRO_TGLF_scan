...
模板管理器 **1.2.4** 修复 ZIP 入口顺序，避免 OMFIT 到案例子目录寻找 `OMFITsave.txt`。更新页提供“修复 ZIP 入口…”按钮，可保留已经生成的工程全部内容并另存可加载的新 ZIP。保留已有模块更新、缺失模块添加和 Python 3.9 字体兼容修复。
...
如果已经生成的 ZIP 打不开，在新版独立管理器的更新页选择该 ZIP，然后点击“修复 ZIP 入口…”；无需重新计算或选择模板。修复只整理归档入口，全部文件内容经 SHA-256 校验，原 ZIP 保留。也可运行 `python3 OMFITtemplates/launch.py repair /path/broken.zip /path/fixed.zip`。需要额外一份完整工程的磁盘空间。仅修复入口不会升级工程内的旧管理器代码，后续更新请继续使用 1.2.4 管理器。
