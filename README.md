...
当前分发版为 **1.7.0**，仅包含代码、输入模板和默认设置；计算案例、结果、缓存与旧命令记录不入库。原模块的输入示例保留在 `TEMPLATES` 中。执行计算前，需要导入自己的平衡/剖面或案例，并配置计算环境与求解器路径。
...
工程 **1.6.1** 修复 OMFIT 原生对比绘图、Project 总控和 TGLF 多输入页面的文字拥挤：按实际中文字体统一字号与行高，增加分组和控件间距，窄窗口中的同行按钮与选项自动换行，长页面使用 OMFIT 滚动区。修复只作用于这些项目页面，不修改 OMFIT 全局字体或主题。内置管理器升级至 **1.9.0** 后，可拉取工程 **1.7.0** 并点击“更新当前工程”，原窗口自动刷新。先使用“检查管理器更新”安装增量更新，随后更新工程模板。
...
当前模板包含 `CGYRO_TGLF_scan` 和 `OMFITtemplates` 两个模块，不带结果或计算案例。内置管理器 **1.9.0** 拉取工程 **1.7.0** 后，选择保留当前案例、结果和设置，点击“预览变更 → 更新当前工程”即可在当前会话生效。未保存的案例和结果也保留原对象，窗口自动刷新。更新时不保存或重载工程，不改变当前项目文件名；照常使用 OMFIT 保存即可。代码或设置在预览后发生变化时要求重新预览；赋值或 GUI 刷新失败会回退本次节点修改。外部独立管理器保持原有的生成新 ZIP 流程。
