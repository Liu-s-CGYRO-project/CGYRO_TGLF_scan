...
1.2.4 修复工程 ZIP 的入口顺序；更新页增加“修复 ZIP 入口”，可修复之前生成的工程。
...
某些 OMFIT 版本根据 ZIP 第一个文件的目录寻找 OMFITsave.txt。旧管理器先写入
案例文件，导致 OMFIT 错到 Cases、GUIS 等子目录寻找工程入口。1.2.4 将实际的
OMFITsave.txt 写在第一个条目，更新工程和无结果分发包均采用此顺序。

1. 在“更新 / 切换”页，把已经生成但打不开的 ZIP 选为“当前工程 ZIP”。
2. 点击右上角“修复 ZIP 入口…”，选择新的输出文件名。不需要选择模板。
3. 等待复制和逐文件 SHA-256 校验完成，再在 OMFIT 中打开新 ZIP。

修复保留全部文件内容、目录、案例、结果和设置，原 ZIP 保留。需要容纳一份完整
工程的额外磁盘空间；可取消操作，未完成的输出会清理。修复不会更新 ZIP 内的
旧管理器代码；后续更新请使用本次安装的独立管理器或更新到包含 1.2.4 的模板。

也可以在无桌面的 Linux 终端运行：

::

    python3 OMFITtemplates/launch.py repair /path/broken.zip /path/fixed.zip

修复针对入口顺序，不会重建实际缺失或损坏的计算文件；发现 CRC 或内容校验失败
会停止并保留原 ZIP。
