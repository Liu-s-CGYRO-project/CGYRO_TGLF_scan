...
模板管理器更新为 **1.2.4**，修复生成 ZIP 后 OMFIT 到案例子目录寻找工程入口的问题。
...
- **111 项 Linux 管理器测试通过**（Ubuntu 22.04 / Python 3.10 / Tk 8.6）。新增覆盖原生入口规则、保留与示例两种更新策略、完整文件内容与元数据保留、单层目录包装、ZIP64、复制/校验取消、源文件变化、CRC/哈希错误以及真实 Tk 修复按钮和无显示 CLI。
- 修复命令使用标准 `zipfile` 流式复制，并读回逐文件校验 SHA-256；不执行工程表达式，不反序列化结果，不覆盖输入或已有输出。GUI 和 CLI 共享同一修复函数。
...
- 使用本地 OMFIT 源码中未修改的 `cherry_pick_OMFITsave` 函数，复现案例子目录入口错误，并验证修复后工程、更新工程及无结果包的完整/选择性解压入口。此项直接调用原生函数，但不初始化完整 OMFIT 桌面会话。
- 以前的自有 Project 解析器能在任意条目顺序找到入口，不能代表原生 OMFIT 已成功加载。此次单元测试明确覆盖该差异；完整服务器 OMFIT 会话及求解器运行仍需在使用环境中验收。
